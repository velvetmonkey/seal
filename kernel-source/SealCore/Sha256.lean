...
namespace SealCore.Sha256

/-- 256-bit SHA-256 digest as the eight FIPS output words. -/
structure Digest256 where
  ...
  w6 : UInt32
  w7 : UInt32
  deriving BEq, DecidableEq, Hashable, Repr, Inhabited, ReflBEq, LawfulBEq

/-- Capability target commitments are 256-bit SHA-256 digests. -/
abbrev TargetHash := Digest256
...
private def sha256State (input : ByteArray) : Array UInt32 := Id.run do
...
/-- SHA-256 digest in the FIPS eight-word output shape. -/
def sha256Digest (input : ByteArray) : Digest256 :=
  let state := sha256State input
  { w0 := state[0]!, w1 := state[1]!, w2 := state[2]!, w3 := state[3]!,
    w4 := state[4]!, w5 := state[5]!, w6 := state[6]!, w7 := state[7]! }
...
private def wordHex (word : UInt32) : String := Id.run do
  let mut s := ""
  for shift in [0:8] do
    let n := ((word >>> (UInt32.ofNat ((7 - shift) * 4))).toNat) % 16
    s := s.push (hexDigit n)
  return s

/-- 64-character lowercase hex encoding of a digest. -/
def Digest256.toHex (digest : Digest256) : String :=
  wordHex digest.w0 ++ wordHex digest.w1 ++ wordHex digest.w2 ++ wordHex digest.w3 ++
  wordHex digest.w4 ++ wordHex digest.w5 ++ wordHex digest.w6 ++ wordHex digest.w7
...
private def wordOfDigits (digits : List Nat) (off : Nat) : UInt32 :=
  UInt32.ofNat <| (digits.drop off |>.take 8).foldl (fun acc d => acc * 16 + d) 0

/-- Parse a strict lowercase 64-hex SHA-256 digest. -/
def Digest256.parseHex? (s : String) : Option Digest256 := do
  let chars := s.toList
  if chars.length != 64 then none else
    let digits ← chars.mapM hexVal?
    some {
      w0 := wordOfDigits digits 0, w1 := wordOfDigits digits 8,
      w2 := wordOfDigits digits 16, w3 := wordOfDigits digits 24,
      w4 := wordOfDigits digits 32, w5 := wordOfDigits digits 40,
      w6 := wordOfDigits digits 48, w7 := wordOfDigits digits 56
    }

/-- 64-character lowercase hex encoding of the digest. -/
def sha256Hex (input : ByteArray) : String :=
  (sha256Digest input).toHex
...
/-- info: true -/
#guard_msgs in #eval
  (Digest256.parseHex? (sha256HexStr "abc") |>.map Digest256.toHex)
