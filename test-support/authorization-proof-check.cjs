...
function run(command, args, cwd, env = process.env) {
  ...
  if (result.error || result.status !== 0) {
    throw new Error(
      `${command} ${args.join(" ")}: ${result.error || result.stderr || result.stdout || result.signal}`,
    );
  }
  ...
  const entries = listing
    ...
    .map((entry) => {
      ...
      if (/\.(olean|ilean|olean\.private|olean\.server)$/.test(name)) {
        throw new Error(
          `source-check: tracked precompiled Lean input: ${name}`,
        );
      }
...
function generatedInputs(root, prefix = "") {
  ...
  return fs
    ...
    .flatMap((entry) => {
      ...
      if (entry.isDirectory()) return generatedInputs(file, name + "/");
      if (!entry.isFile())
        throw new Error(`source-check: nonregular generated input ${file}`);
      return [
        {
          name,
...
function checkProof({ sourceRoot, evidencePath }) {
  const evidence = {
    schema: "seal.authorization-source-proof/v1",
    scope: "source-only",
    result: "FAIL",
    theorems: THEOREMS,
    axiom_allowlist: ["propext", "Classical.choice", "Quot.sound"],
    residual_assumptions: [
      "Lean checker and toolchain",
      "compiler",
      "runtime",
      "IO",
    ],
  ...
  try {
    ...
    const work = fs.mkdtempSync(
      path.join(path.dirname(evidencePath), "authorization-proof-"),
    );
    ...
    run(
      "git",
      ["clone", "--no-local", "--no-checkout", sourceRoot, checkout],
      work,
    );
    ...
    const toolchain = fs
      .readFileSync(path.join(checkout, "lean-toolchain"), "utf8")
      .trim();
    ...
    if (!evidence.lean_version.startsWith(`Lean (version ${toolchain.split(":v")[1]},`)) {
      throw new Error("source-check: Lean version does not match the pinned toolchain");
    }
    evidence.lake_version = run("lake", ["--version"], checkout, env);
    evidence.node_version = process.version;
    evidence.npm_version = run("npm", ["--version"], checkout, env);
    ...
    const buildArgs = [
      "--no-cache",
      "build",
      "AuthorizationCorrespondenceProof",
    ];
    ...
    let build;
    try {
      // Resolve the locked source dependencies without compiling their Lean
      // libraries. ProofWidgets' Mathlib configuration refuses a cold widget
      // build, so run that package's own JS source target in its own context.
      // This produces widget assets, not downloaded Lean build artifacts.
      const phases = [
        { command: "lake", args: ["--no-cache", "env", "true"], cwd: checkout },
      ];
      if (
        manifest.packages.some(
          (pkg) => pkg.name.replace(/^«|»$/g, "") === "proofwidgets",
        )
      ) {
        phases.push({
          command: "lake",
          args: ["--no-cache", "build", "widgetJsAll"],
          cwd: path.join(checkout, manifest.packagesDir, "proofwidgets"),
        });
      }
      ...
      for (const phase of phases) {
        ...
        evidence.preparation.push({ ...phase, exit: result.status });
        if (result.error || result.status !== 0)
          throw new Error(
            `source-check: preparation failed: ${phase.args.join(" ")}: ${result.error || result.status}`,
          );
      }
      ...
      evidence.transcript = {
        path: transcript,
        sha256: sha256(fs.readFileSync(transcript)),
      };
    }
    evidence.build_exit = build.status;
    if (build.error || build.status !== 0) {
      throw new Error(
        `source-check: proof build failed: ${build.error || build.signal || build.status}; see ${transcript}`,
      );
    }
    ...
    for (const name of THEOREMS) {
      if (!log.includes(`correspondence proof: ${name}:`)) {
        throw new Error(
          `source-check: missing fresh axiom-check output for ${name}`,
        );
      }
    }
    if (
      sourceIdentity(checkout).sha256 !== source.sha256 ||
      sha256(fs.readFileSync(manifestPath)) !== evidence.manifest_sha256
    ) {
      throw new Error(
        "source-check: build changed committed source or dependency manifest",
      );
    }
    evidence.dependencies = manifest.packages.map((pkg) => {
      if (pkg.type === "path") {
        const local = path.resolve(checkout, pkg.dir);
        if (!local.startsWith(checkout + path.sep))
          throw new Error("source-check: external path dependency");
        return {
          name: pkg.name,
          type: "path",
          path: pkg.dir,
          covered_by: source.sha256,
        };
      }
      ...
      if (identity.commit !== pkg.rev)
        throw new Error(`source-check: dependency revision mismatch: ${name}`);
      return { name, url: pkg.url, ...identity };
    });
    evidence.generated_widget_inputs = generatedInputs(
      path.join(checkout, manifest.packagesDir, "proofwidgets/.lake/build/js"),
    );
    evidence.closure_sha256 = sha256(
      JSON.stringify({
        source: source.sha256,
        dependencies: evidence.dependencies,
        toolchain,
        lean_version: evidence.lean_version,
        lake_version: evidence.lake_version,
        node_version: evidence.node_version,
        npm_version: evidence.npm_version,
        generated_widget_inputs: evidence.generated_widget_inputs,
      }),
    );
...
if (require.main === module) {
  try {
    ...
    if (!values["source-root"] || !values.evidence)
      throw new Error("required: --source-root --evidence");
    const evidence = checkProof({
