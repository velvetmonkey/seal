...
function committedPopulation() {
  ...
  for (const key of POPULATION_KEYS) {
    ...
    if (!match) throw new Error(`missing integer ${key}`);
    population[key] = Number(match[1]);
  }
...
async function measure({ sourceRoot, sourceFiles, write, allowShrink }) {
  ...
  if (write) {
    ...
    const changes = populationChanges(oldPopulation, newPopulation);
    const shrinks = changes.filter(({ difference }) => difference < 0);
    if (shrinks.length && !allowShrink) {
      console.error(`REFUSE link-check population shrink: ${describeChanges(shrinks)}; rerun with --write --allow-shrink to accept a legitimate deletion`);
      return 1;
    }
    if (shrinks.length) {
      console.log(`OVERRIDE link-check population shrink: ${describeChanges(shrinks)}`);
    }
    const newSource = populationSource(newPopulation);
