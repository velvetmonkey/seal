...
function legacyReadmeRegions({ manifest, manifestPublished }) {
  ...
  return [
    [
      ...
      `&& curl -fsSLO "https://github.com/velvetmonkey/seal/releases/download/$SEAL_VERSION/${manifest.checksums.name.replace(manifest.tag, "$SEAL_VERSION")}" \\`,
      ...
      `&& curl -fsSLO "https://github.com/velvetmonkey/seal/releases/download/$SEAL_VERSION/${manifest.checker.name.replace(manifest.tag, "$SEAL_VERSION")}" \\`,
