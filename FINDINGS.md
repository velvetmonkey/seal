...
| Seal proves properties of the mediation KERNEL, not of the whole deployed system. | Yes (documented + true) | docs/LIMITATIONS.md (verbatim), docs/CLAIMS-MATRIX.md, seal-host/docs/LIMITATIONS.md + TCB.md | keep |
| The deployed Rust / wasm / JS are NOT proven bug-free; they are tied to the proof by byte-exact conformance testing over a corpus, not for every possible input. | Yes (documented + true) | docs/LIMITATIONS.md (verbatim block), CLAIMS-MATRIX "Tested" rows + conformance transcripts | keep |
| Seal guarantees AUTHORIZATION match, not INTENT match: if a human approves a malicious-but-valid request, Seal will execute it. | Yes (documented + true) | docs/LIMITATIONS.md (verbatim), README "honest claim" sections, EVALUATOR-START | keep |
| Seal's audit chain is tamper-EVIDENT, not tamper-IMPOSSIBLE. | Yes (documented + true) | docs/LIMITATIONS.md (verbatim), seal-host Host/Record.lean (tamper_evident theorem with hypotheses) | keep |
...
| SHA-256 collision resistance (A-CR). | Assumed (named, scoped) | docs/LIMITATIONS.md, family TCB docs | keep |
...
| Intent match / making the AI smarter / preventing hallucinations. | Not claimed | docs/LIMITATIONS.md (verbatim) | keep |
...
See also the canonical CLAIMS-MATRIX.md (one table view) and docs/LIMITATIONS.md (verbatim non-claims block, drift-guarded).
