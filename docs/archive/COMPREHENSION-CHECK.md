...
a JavaScript `number`. `OPEN-docs/archive/findings.md` row 40 records the concrete failure:
...
`OPEN-docs/archive/findings.md` rows 35 and 40 establish on disk that the recorded kernel rule
...
`OPEN-docs/archive/findings.md` rows 36 and 37 support the Lean type limitation and the
...
2. The earlier definition `digest = sha256(bytes)` is not precise enough for
   the approval target. `OPEN-docs/archive/findings.md` row 24 says the target is computed by
...
`OPEN-docs/archive/findings.md` rows 24, 35-37 and 40. The unavailable implementation
