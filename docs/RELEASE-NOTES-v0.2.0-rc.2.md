...
**Named limit — local configuration authority.** Seal does not defend against a local actor who can already run commands as the developer: it refuses forwarding when the recorded project-server entry drifts, and refuses `unprotect` when the local override no longer matches Seal's recorded definition, but that actor can start Claude Code with a different `CLAUDE_CONFIG_DIR`, edit or replace the local entry, or run the server directly. Seal binds the route a Claude Code session takes through its protected MCP entry; it does not promise to secure the machine or prevent a user-equivalent local actor from choosing another route. ([local installation and ownership checks](../spine/protection.cjs); [ownership-preservation test](../test/protect3b.test.cjs))
