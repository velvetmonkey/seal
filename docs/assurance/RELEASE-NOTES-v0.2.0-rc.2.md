...
The published release payload includes the checker; it is not a separate release asset, so it cannot independently authenticate that payload. That limitation never lifts for this checker, because it checks receipt contents rather than the bytes of the payload that contains it; to authenticate the payload, verify its artifact digest against an independently obtained release pin. It does not import Seal at check time, but its canonicalisation is a copy of the product-side implementation. ([artifact payload](../../scripts/build-dist.cjs); [release workflow](../../.github/workflows/release.yml); [checker limits](../../checker/seal-receipt-check.mjs); [checker test](../../test/receipt-checker.test.cjs); merged [18bba8e](https://github.com/velvetmonkey/seal/commit/18bba8ea230ead9fb605cd61d352a0e894c256d5))
