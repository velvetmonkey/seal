...
> For the truth about what you installed, read [release evidence](RELEASE-NOTES-v0.2.1.md) and the [README](../../README.md).
...
flowchart LR
    ...
    subgraph kernel["Decision core — proven [seal-host] in Lean 4 (seal-host)"]
...
- **Decision core** (`seal-host`, the repository holding its Lean source) — the seal-host rulebook. The seal-host kernel has machine-checked [seal-host] Lean 4 theorems: default
  deny, allow **iff** a live approval record matches the exact target, single-use, expiry,
  non-bypass. The seal-host kernel is proven [seal-host] — but this is a *kernel* claim, not a whole-system claim; that the record was
