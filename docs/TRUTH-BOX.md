...
> **Family framing:** Seal is the agent authorization gate whose decision rule is machine-checked, whose effect commitment is tested for sufficiency, and whose deployed decisions can be independently re-derived against pinned kernel bytes. This umbrella is the family spine; the sibling repositories each serve one clause. Seal protects effects. The current production adapter mediates MCP `tools/call`. The gate enforces authorization at the effect boundary and does not claim to read intent.
