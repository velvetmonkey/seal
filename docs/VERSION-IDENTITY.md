...
the artifact digest in-tree (`SHA256SUMS`, quoted again in `README.md` and
`docs/DISTRIBUTION.md`). A digest that depended on the commit could never be
pinned by a commit: writing the pin would change the commit, which would change
the digest, which would need a new pin. So the payload is named by `VERSION`
...
2. a branch is opened from the released commit with `VERSION` set to the next
   intended release, `scripts/sync-version.cjs` run, and the artifact pin
   regenerated by `scripts/repin-dist.cjs`;
