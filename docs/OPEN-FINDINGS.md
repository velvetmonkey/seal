...
Consolidated 2026-07-25; updated 2026-07-26. Everything currently known-broken
...
| 12 | **`wireNumbersSafe` measures the wrong thing.** It bounds the exponent to six decimal digits, so `1e999999` is ADMITTED. The claim is "every admitted exponent is safe to parse"; the measurement is "every exponent spelled with at most six digits is admitted". Needs subprocess-isolated, timeout-bounded tests through interpreted Lean, native FFI, wasm and the deployed host, with the bound derived from the largest observed-safe value. | OPEN |
...
| 23 | **`feat/passthrough-perimeter` carries finished work this ledger never recorded, and main carries the over-claim it retracts.** Four commits, ~828 lines, 2026-07-25, in no row until now. It characterises the mediation perimeter, records a widened-alphabet NON-BYPASS FAILURE verdict, and renames `deployedAdapter` -> `gatedSinkAdapter` (K3) because the alias covers only the gated child-input sink (P2 forward, P3 retry), NOT P1 passthrough or P4-P9. **`CLAIMS.md` on main therefore asserts "deployed adapter by name" for a model proved to cover two transitions of nine; killing this branch preserves that over-claim rather than the status quo.** Static audit: `sorry` 0, `admit` 0, `native_decide` 0 and banned in-file, custom axioms 0, every theorem `#guard_msgs`-pinned, 5 audit pins added to `Test/Axioms.lean`, 53 `#guard` witnesses. Conflict-free, outside the repin razor. Harvest conditions: run `perimeter_probe` with a deliberate sabotage and record the RED in the merge trailer (a convention main adopted two days AFTER this branch was cut), and a full lake build to confirm the pins still pass. | **NEEDS BEN** |
