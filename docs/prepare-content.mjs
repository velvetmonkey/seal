...
function main() {
  ...
  for (const source of sources) {
    ...
    // Starlight renders its own <h1> from this frontmatter title. Leaving the
    // source's own "# Heading" line in the body duplicated that title on
    // every rendered page. Drop only that first matching line here, in the
    // site build step; the repository's own Markdown file keeps its heading
    // for GitHub, README rendering and every other non-Starlight reader.
    const titleMatch = original.match(/^#\s+(.+)$/m);
    const sourceTitle = titleMatch?.[1]?.replace(/[*_`]/g, '') || path.basename(source, '.md');
    const withoutH1 = titleMatch ? original.replace(`${titleMatch[0]}\n`, '') : original;
    ...
    const prepared = prepareMarkdown(sourceName, withoutH1);
    fs.writeFileSync(destination, `---\ntitle: ${JSON.stringify(title)}\n${archiveFrontmatter}---\n\n${warning}${rewriteLinks(source, prepared)}`);
  }

  console.log(`Prepared landing page and ${sources.length} markdown sources; each keeps its own repository heading, minus the body's leading title line duplicated into Starlight's frontmatter`);
}
