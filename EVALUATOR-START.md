...
Snapshot: 2026-07-31. This file separates proved, tested, reproduced, shipped,
and open facts. It is a hand-generated snapshot from the two `CLAIMS.md` files,
the fleet lock, the live verifier declarations, and the current proof-reference
inventories. It is not yet generated by a checked-in tool. That is a stop-ship.

> **Dated correction, measured 2026-08-08.** Section 2's artifact-identity rows
> describe the 2026-07-31 state and are superseded on these points: the fleet
> lock (`seal-host/release/fleet-lock.json`) now records kernel SHA-256
> `0b5e792500592b56847f70b1e27e47aecdc65023c7c59fd79695102c465f26ec`, and all
> six locked fleet wasm paths were independently re-hashed to that same value —
> the shipped fleet is no longer `d7d81e27`, and the checked-in
> `seal-host/wasm-spike/verified/seal.wasm` also hashes `0b5e7925…`. The
> `dd00cd2b` clean-rebuild candidate row remains a true record of that rebuild.
> No GitHub release has been published for any tag as of this date. The rest of
> this file is retained verbatim as the 2026-07-31 audit.
...
## 2. Artifact identity: candidate, reproduction, and shipped fleet
...
## 5. Current theorem inventories and the red inventory gate
...
The module axiom gate is currently RED. Executing the existing built gate from
`mcp-seal-dev` exits 1 with production modules `51/50` and kernel-baseline
assignments `25/24`. Backing: `Test/ModuleAxiomScan.lean:35-100`; 47 Lean files
under `Seal`, `SealCore`, and `SealV2`, plus four root modules, were counted on
disk. Therefore this surface does not repeat the prior unqualified
"axiom-clean" umbrella claim. Individual theorem footprints remain those
recorded by their own gates; the full module inventory gate needs repair.

## 6. Five stop-ships

| Stop-ship | Current measured state | Backing and closure condition |
|---|---|---|
| Coordinated wasm/fleet promotion | OPEN, MEASURED. CANDIDATE `dd00cd2b` and SHIPPED `d7d81e27` differ | Section 2 hashes; promote candidate and fleet together, or keep the gap explicit |
| Umbrella regeneration | OPEN, MEASURED. This file is hand-maintained and the prior surface had at least thirteen stale or under-scoped assertions | This rewrite's source audit; no generator exists under `seal/scripts`, while `scripts/linkcheck.mjs:11` only link-checks this file; closure requires checked regeneration from both CLAIMS files, the lock, and live profiles |
| Passthrough perimeter choice | OPEN, MEASURED. The model proves P1 escape bypass; no option is chosen | `seal-host/CLAIMS.md:44`; `Host/PassthroughPerimeter.lean:615-684`. Option A: refuse every escape before the child, paying protocol compatibility cost. Option B: retain declared passthrough only for a named adapter and strict-child profile with executable per-adapter evidence, paying a permanent adapter-specific TCB and test burden. Neither option has been chosen |
| Release gating at the exact tagged commit | OPEN, MEASURED. The tag workflow runs fleet and packaging gates, but does not require the full deterministic acceptance workflow or the CI `release-evidence` result at that same tag | `seal-host/.github/workflows/release.yml:4-216`; acceptance is a separate push workflow at `.github/workflows/golden-path.yml:3-10`; CI aggregation is `.github/workflows/ci.yml:476-507`. Closure requires full acceptance to gate publication for the exact tagged commit |
| Public export buildable with zero private access | OPEN, MEASURED. The successful clean rebuild required a host GitHub credential for pinned private revisions | `/var/tmp/seal-wasm-clean-final2.2ia1TA/REPRODUCE.md:27-46`; `reproduce.sh:11-17`. Closure requires the public export to build with no private authorization |

All five rows describe measured current conditions. Their successful closures
remain future work and were not performed under this semantic freeze.
...
- Universal parser or conformance coverage beyond the named corpora: not
  checked.
- A public build with zero private access: not checked and currently blocked by
  the clean recipe's credential requirement.
